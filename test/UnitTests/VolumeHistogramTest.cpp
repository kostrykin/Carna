...
struct ConcreteCounterVolumeHistogramTest
{
    ...
    typedef typename Carna::base::model::VolumeHistogram< ConcreteCounter > VolumeHistogram;
...
private:
    ...
    static Carna::base::model::UInt16Volume* createVolume( const std::map< signed short, unsigned int >& frequencies, signed short padding_value );
...
{
    const std::unique_ptr< Carna::base::model::UInt16Volume > volume( createVolume( frequencies, padding_value ) );
...
template< typename ConcreteCounter >
Carna::base::model::UInt16Volume* ConcreteCounterVolumeHistogramTest< ConcreteCounter >::createVolume( const std::map< signed short, unsigned int >& frequencies, signed short padding_value )
{
    const unsigned int minimum_voxel_count = std::accumulate
        ( frequencies.begin()
        , frequencies.end()
        , 0u
        , []( unsigned int total, const std::map< Carna::base::model::UInt16Volume::VoxelType, unsigned int >::value_type& entry )->unsigned int
        {
    ...
    Carna::base::model::UInt16Volume::BufferType* const buffer = new Carna::base::model::UInt16Volume::BufferType( volume_size.x * volume_size.y * volume_size.z, Carna::base::model::UInt16Volume::HUVToBufferValue( padding_value ) );
    Carna::base::model::UInt16Volume* const volume = new Carna::base::model::UInt16Volume( volume_size, new Carna::base::Composition< Carna::base::model::UInt16Volume::BufferType >( buffer ) );

    unsigned int voxel_offset = 0;
    for( auto frequencies_itr = frequencies.begin(); frequencies_itr != frequencies.end(); ++frequencies_itr )
    {
        const Carna::base::model::UInt16Volume::VoxelType voxel_value = Carna::base::model::UInt16Volume::HUVToBufferValue( frequencies_itr->first );
...
void VolumeHistogramTest::testWithIndexedCounter()
{
    try
    {
        ConcreteCounterVolumeHistogramTest< Carna::base::model::IndexedCounter< signed short > >().run();
    }
...
void VolumeHistogramTest::testWithAssociativeCounter()
{
    try
    {
        ConcreteCounterVolumeHistogramTest< Carna::base::model::AssociativeCounter< signed short > >().run();
    }
    catch( const std::out_of_range& ex )
    {
        QFAIL( ex.what() );
