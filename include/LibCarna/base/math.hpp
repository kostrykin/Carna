...
namespace math
{
    ...
    inline Vector3f orthogonal3f( const Vector3f& in )
    {
        ...
        /** Obviously the vector was too short. Check this. If it was not, than
          ...
          */
        LIBCARNA_ASSERT( isEqual< float >( in.dot( out ), 0 ) );
        return out;
    }
    ...
    {
        LIBCARNA_ASSERT( isEqual< float >( normal.norm(), 1 ) );
    ...
    {
        LIBCARNA_ASSERT( !std::numeric_limits< ScalarType >::is_signed || x >= 0 );
