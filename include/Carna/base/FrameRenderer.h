...
/** \brief
  ...
  *
  *     A frame renderer mainly defines a \em sequence of rendering stages. Each
  *     rendering stage contributes several rendering instructions to the overall
  *     rendering process. Such instructions can be the rendering of 3D objects or
  *     also something completely different, like taking care of stereoscopic
  *     rendering.
  *
  *     The rendering process is made up by the execution of \em render \em tasks,
  *     that are nested occasionally. At the beginning of the process there is always
  *     exactly one render task. It's purpose is to invoke the rendering stages of
  *     the frame renderer successively.
  *
  ...
  *
  *     However, there is an important concept about render tasks, which allows
  *     realization of more complex rendering processes: When triggered from a render
  *     task, a rendering stage may
  *     \ref RenderTask::RenderTask(const RenderTask&, Framebuffer&) "fork" the task
  *     it was invoked by. This is how render tasks can become nested. Such a forked
  *     task is basically a clone of the original one: It continuous processing
  *     rendering stages with the next stage that it was created by. It's purpose
  *     usually is to create a rendering from the succeeding stages to a texture,
  *     that is somehow processed afterwards. When finished, the original task
  *     continuous with the same.
  *
