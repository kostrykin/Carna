...
#include <Carna/VolumeRenderings/DVR/ColorMapTrackerEditor.h>
#include <QPainter>
...
void ColorMapEditor::processTrackerMousePress( base::IntSpan& span, int index, base::qt::MultiSpanSliderTracker::Role mode )
{
...
void ColorMapEditor::processNewSpan( base::IntSpan&, int index )
{
    QColor color0( nextColor );
    QColor color1( nextColor );

    ++nextColor;

    color0.setAlphaF( 0.0f );
    color1.setAlphaF( 0.3f );
...
void ColorMapEditor::processRemovedSpan( base::IntSpan& span, int index )
{
...
void ColorMapEditor::processModifiedSpan( base::IntSpan& span, int index )
{
    if( selectedSpan == &span )
    {
        if( beginConfigWidget )
...
{
    base::qt::MultiSpanSlider::paintEvent( ev );
