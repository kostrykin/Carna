...
bool MultiSpanSlider::canTakeUpdatedSpan( unsigned int index, const IntSpan& span ) const
{
...
void MultiSpanSlider::updateSpan( unsigned int index, const IntSpan& span )
{
...
unsigned int MultiSpanSlider::addSpan( const IntSpan& span )
{
    IntSpan adjusted_span = span;
    adjusted_span.first = Math::clamp< int >( span.first, this->first, this->last );
    adjusted_span.last  = Math::clamp< int >( span.last , this->first, this->last );
...
void MultiSpanSlider::mouseMoveEvent( QMouseEvent* ev )
{
    if( grabbedIndex == -1 )
    {
        ...
        try
        {
            ...
            IntSpan new_span( mvm.pixelToValue( y0 ), mvm.pixelToValue( y1 ) );
    ...
    IntSpan& real_span = spans[ grabbedIndex ];

    const IntSpan orig_span = real_span;
    IntSpan new_span = real_span;
